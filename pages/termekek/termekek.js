...
function katKeszit() {
  for (let index = 0; index < termekek.length; index++) {
    const aktualisTermek = termekek[index];
    const aktualisKategoria = aktualisTermek.kategoria;
    for (let index = 0; index < aktualisKategoria.length; index++) {
      if (!kategoriak.includes(aktualisKategoria))
        kategoriak.push(aktualisKategoria);
    }
  }
}
